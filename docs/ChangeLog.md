...
- **New Event** captures: title, customer, project (optional — an event doesn't have to be tied to one), type (Catch-up / Technical / Account / Scoping / Training / Other), format (Video call / In person / Phone call / Other), date, status (Scheduled / Logged), attendees (free text), time spent (optional hours), key items, actions, and an "Internal only" checkbox to keep a note off the customer's Portal.
