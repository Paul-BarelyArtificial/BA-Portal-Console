...
## v0.2.8d — Backfill Upload Quota Field
- The Storage rules fix in v0.2.8c relied on `.get("uploadStorageUsedBytes", 0)` gracefully treating a missing field as zero — but real-world testing still failed the same way after republishing, so that specific method may not be reliable when called on data fetched via Storage rules' `firestore.get()` bridge (a newer, more limited feature than native Firestore rules).
- Rather than keep guessing at rules-language behaviour I can't test myself, removed the uncertainty at the source: the Console now automatically backfills `uploadStorageUsedBytes: 0` onto any customer record that doesn't already have it, every time the Customers list loads. This runs alongside the existing `customerAccess` sync, so simply opening the Console (Customers or Dashboard) brings every existing customer's record up to date.
- This guarantees the field always exists as an actual number by the time any customer attempts an upload, regardless of what the Storage rules can or can't handle gracefully.

## v0.2.8e — Simplify Storage Quota Rule
- Real-world testing kept failing with `storage/unauthorized` even after two targeted fixes to the cumulative-quota check, on files nowhere near either limit. Rather than keep guessing at Storage rules syntax I have no way to test myself, removed the cross-service cumulative-total check from the Storage rule entirely.
...
## v0.9.0 — Events & Time Tracking (Console v0.9.0 / Portal v0.5.0)
- **Replaces Time Tracker entirely** with a new **Events** section: a record of every meeting, call or session with a customer, not just billable time. Existing `timeSessions` data and the "Time & Billing" hours-per-day setting are untouched and still count towards project time totals — only the logging workflow moved.
- **New Event** captures: title, customer, project (optional — an event doesn't have to be tied to one), type (Catch-up / Technical / Account / Scoping / Training / Other), format (Video call / In person / Phone call / Other), date, start time (optional), status (Scheduled / Logged), attendees (free text), time spent (optional hours), key items, actions, and an "Internal only" checkbox to keep a note off the customer's Portal.
...
- **Requires a Firestore rules update** — `docs/firestore.rules.txt` now includes an `events` rule (admin full access; a customer can read their own non-internal-only events). Must be published before this feature works.
- Included in the Console's Data Export.
