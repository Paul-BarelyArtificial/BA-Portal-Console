...
## ~~Marketing Opportunities: recurring cost field and usefulness rating~~ (requested and built 2026-09-10)

Two small additions to each Marketing Opportunity, built together since they touch the same dialog/table:

- **Cost** — an optional amount plus a frequency (One-off / Per month / Per year, defaulting to Per month), e.g. "£25 per month" for a networking breakfast. Shown as its own column and in the detail panel; formatted via the existing `formatCurrency()` helper.
- **Usefulness rating** — an optional 1–5 field ("1 — Won't be much use" to "5 — Will be really useful"), rendered as ★★★★☆ (4/5) in the table and detail panel.

Both fields are optional and default to unset/Not rated on existing records. Sorting/filtering by rating was intentionally left out of this pass — noted in Backlog if wanted later.

## ~~Company Dashboard~~ (requested and built 2026-09-09)

Paul wanted one place to jump straight to every company-wide tool and service (apps, Calendly, QuickBooks, etc.) instead of hunting for the URL each time. Built as a new top-level Console section:

- New `companyLinks` Firestore collection (admin read/write only) — `name`, `url`, `category`.
- Links render grouped into category sections. Category is a free-text field with autocomplete suggestions drawn from categories already in use, the same pattern as Library's `collection` field — so the category list stays configurable without needing a separate management screen.
- Each link card shows the site's favicon (fetched client-side from Google's favicon service by hostname — nothing stored) next to its name; clicking a card opens the link in a new tab. Edit/Delete icon-buttons appear on hover.

Shipped as Console v1.1.0. Firestore security rule deployed to the live `barely-crm-dev` project and verified end-to-end (signed in as the demo admin, added QuickBooks and Calendly as real starter links).
...
- **Light/dark theme toggle — Portal side only** (parked 2026-08-04, Console side built) — **Console already has this**: a light/dark/auto toggle in the header (`js/app.js` `applyTheme`/`THEME_STORAGE_KEY`), with `[data-theme="light"]` CSS overrides throughout `css/styles.css`. Portal (`BA-Portal-Dev`) does not — it still only has its single dark palette (`--bg-main`, `--bg-panel`, `--text-main`, etc.), no toggle UI and no light-mode overrides. Remaining work is Portal-only: add the same toggle control, `[data-theme="light"]` variable overrides, and convert whatever hardcoded colours bypass Portal's CSS variables — likely a much smaller job now that Console's version exists as a reference implementation to copy the pattern from.
